...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex !== -1) {
    const beforeLastOccurrence = str.slice(0, lastIndex);
    const afterLastOccurrence = str.slice(lastIndex + value.length);
...
function countVowels(str) {
  ...
  const vowelArray = str.split('').filter((char) => vowels.includes(char));
...
function findLongestWord(sentence) {
  const longestWord = sentence.split(' ').reduce((prevWord, currentWord) => {
...
function invertCase(str) {
  const invertedString = str.split('').map((char) => {
    return char === char.toUpperCase()
      ? char.toLowerCase()
      : char.toUpperCase();
  });

  // З'єднуємо обернуті символи назад у рядок та повертаємо результат.
...
function getStringFromTemplate(firstName, lastName) {
  const formattedString = `Hello, ${firstName} ${lastName}!`;
...
function unbracketTag(str) {
  const unbracketedTag = str.slice(1, -1);
  return unbracketedTag;
}
/**
...
function extractEmails(str) {
  const emailArray = str.split(';').map((email) => email.trim());
  return emailArray;
}
/**
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    return String.fromCharCode(
      baseCharCode + ((char.charCodeAt(0) - baseCharCode + 13) % 26)
    );
  });
}
/**
...
function getCardId(value) {
  ...
  const cardId = initialDeck.indexOf(value);
  return cardId;
}
module.exports = {
  getStringLength,
